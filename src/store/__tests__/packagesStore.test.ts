...
import { usePackagesStore, selectTemplates } from "../packagesStore";
import {
  packageListCatalog,
...
describe("packagesStore", () => {
  ...
  it("loadCatalog populates catalog and sets fresh status", async () => {
    ...
    expect(usePackagesStore.getState().catalog).toHaveLength(1);
    expect(usePackagesStore.getState().indexStatus).toBe("fresh");
  });

  it("selectTemplates returns only entries with a template", () => {
    usePackagesStore.setState({
      catalog: [
        { name: "a", template: { path: "t", entrypoint: "m.typ" } } as any,
        { name: "b", template: null } as any,
      ],
    });
    const t = selectTemplates(usePackagesStore.getState());
