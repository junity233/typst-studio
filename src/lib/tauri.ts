...
async function browserInvoke<T>(
  ...
  switch (command) {
    ...
    case "update_text":
      return Number(args.revision ?? 0) as T;
    case "reactivate_tab":
...
/**
 * Push one versioned source snapshot to the backend (debounced by caller).
 *
 * `revision` is generated by the frontend on every Monaco content change and
 * survives debounce coalescing. The backend adopts this exact revision and
 * ignores older requests, so an earlier IPC can never overwrite newer text.
 */
export async function updateText(
  ...
  return invoke<number>("update_text", { id, content, revision });
}
