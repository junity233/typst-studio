...
describe("closeTabWithConfirm 'Don't Save' calls discard_recovery (§5.1.4)", () => {
  ...
  it("dirty tab + 'Don't Save' → discard_recovery then close", async () => {
    ...
    useDocumentsStore.getState().upsertDocument({
      ...
      // §17 origin mirror: disk-backed test doc → looseFile rooted at /w.
      origin: { kind: "looseFile", path: "/w/notes.typ", root: "/w" },
      revision: 3,
      conflict: "none",
  ...
  it("dirty tab + 'Save' → does NOT discard recovery", async () => {
    useDocumentsStore.getState().upsertDocument({
      ...
      dirty: true,
      content: "x",
      origin: { kind: "looseFile", path: "/w/saved.typ", root: "/w" },
      revision: 1,
      conflict: "none",
      conflictDiskContent: null,
      status: "idle",
  ...
  it("clean tab → no prompt, no discard_recovery", async () => {
    useDocumentsStore.getState().upsertDocument({
      ...
      dirty: false,
      content: "x",
      origin: { kind: "looseFile", path: "/w/clean.typ", root: "/w" },
      revision: 0,
      conflict: "none",
      conflictDiskContent: null,
      status: "idle",
